...
{
    public class AsyncObservableCollection2<T> : NotifyPropertyChangedBase,
   IList,
        ...
        where T : IEquatable<T>
    {
        ...
        public AsyncObservableCollection2(IEnumerable<T> items)
        {
            ...
            _backingCollection = new List<T>(_hashCheck);
        }
...
#pragma warning restore 8424
        ...
        {
            var args = await _lock.WriteAsync(this, t => DoTransactionImpl(() => action(t)));
            if (args == null)
                return;
        ...
        {
            var newItems = await _lock.WriteAsync(items, async adding =>
            {
