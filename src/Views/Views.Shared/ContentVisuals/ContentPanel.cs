                               ...
                               IContentVisual
   {
                                                    ...
                                                    Int32 y)
      {
         ...
         if (Content is { } content &&
             ...
             content.TryHandleInput(args, x, y))
            return true;
      ...
      protected TQuery QueryContent<TQuery>(Func<IVisualElement, TQuery> query,
                                            TQuery defaultValue)
      {
         return Content is not { } valid ? defaultValue : query(valid);
      }
      ...
      protected virtual void OnContentChanged(IVisualElement? oldValue,
                                              IVisualElement? newValue)
      {
         //UILogger.Log(GetType().Name + "'s Content changed to " + newValue, LogLevel.Level1);

         if (!ReferenceEquals(null, oldValue) &&
             !ReferenceEquals(null, newValue))
         {
            InvalidateMeasure();
         }
