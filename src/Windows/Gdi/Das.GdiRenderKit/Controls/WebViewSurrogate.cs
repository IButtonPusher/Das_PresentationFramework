...
public class WebViewSurrogate : WebView2,
                                IVisualSurrogate
{
   public WebViewSurrogate(HtmlPanel element,
                           Control hostingControl)
   {
      ...
      {
         _pendingMarkup = element.Markup;
      }

      ISupportInitialize iInit = this;
                                                      ...
                                                      CoreWebView2InitializationCompletedEventArgs e)
   {
      lock (_loadLock)
      {
         ...
         if (_pendingMarkup is { } pendingMarkup &&
             !String.IsNullOrEmpty(pendingMarkup))
         {
            CoreWebView2.NavigateToString(pendingMarkup);
         }
         else if (_pendingUri != null)
            CoreWebView2.Navigate(_pendingUri.ToString());
      }
   }
                                         ...
                                         PropertyChangedEventArgs e)
   {
      lock (_loadLock)
      {
         switch (e.PropertyName)
         {
            ...
            case nameof(HtmlPanel.Markup) when !String.IsNullOrEmpty(_htmlPanel.Markup):
               if (_isWebViewLoaded)
                  CoreWebView2.NavigateToString(_htmlPanel.Markup);
               else
                  _pendingMarkup = _htmlPanel.Markup;
               break;

            case nameof(HtmlPanel.Uri) when _htmlPanel.Uri != null:

               if (_isWebViewLoaded)
                  CoreWebView2.Navigate(_htmlPanel.Uri.ToString());
               else _pendingUri = _htmlPanel.Uri;
   ...
   private Boolean _isWebViewLoaded;

   private String? _pendingMarkup;
