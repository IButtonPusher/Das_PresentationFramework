...
namespace Das.ViewModels
{
      ...
      where T : IEquatable<T>
   {
      ...
      public AsyncObservableCollection(IEnumerable<T> items,
                                       ISingleThreadedInvoker ui)
         : base(ui)
      {
         _ui = ui;
         ...
         {
            var bc = new ObservableRangeCollection<T>(_hashCheck);
            Count = bc.Count;
...
#pragma warning restore 8424
                                           ...
                                           IProgress<T> progress)
      {
         ...
         return await DoTransaction(arr, (_,
                                          b) =>
         {
            if (RemoveImpl(b))
               progress.Report(b);
      ...
      public void CopyTo(T[] array,
                         Int32 arrayIndex)
      {
         _copyForSynchronous.CopyTo(array, arrayIndex);
      }
      ...
      {
         return _copyForSynchronous[index];
      }

      public IEnumerator<T> GetEnumerator()
      {
         var arr = _copyForSynchronous;
         foreach (var r in arr)
         {
            yield return r;
         }
      ...
      {
         var copy = _copyForSynchronous;
         for (var c = 0; c < copy.Length; c++)
         {
            if (Equals(copy[c], item))
               return c;
         }
      ...
      private void InsertImpl(Int32 index,
                              T value)
      {
      ...
      private void OnBackingCollectionChanged(Object sender,
      ...
      protected readonly ObservableRangeCollection<T> _backingCollection;
      private T[] _copyForSynchronous;
