...
namespace Das.Views.Windows
{
    internal static class TaskDialogNativeMethods
    {
        ...
        {
            NavigatePage = Native.UserMessage + 101,
            ClickButton = Native.UserMessage + 102, // wParam = Button ID
            SetMarqueeProgressBar = Native.UserMessage + 103, // wParam = 0 (nonMarque) wParam != 0 (Marquee)
            SetProgressBarState = Native.UserMessage + 104, // wParam = new progress state
            SetProgressBarRange = Native.UserMessage + 105, // lParam = MAKELPARAM(nMinRange, nMaxRange)
            SetProgressBarPosition = Native.UserMessage + 106, // wParam = new position
            SetProgressBarMarquee = Native.UserMessage + 107, // wParam = 0 (stop marquee), wParam != 0 (start marquee), lparam = speed (milliseconds between repaints)
            SetElementText = Native.UserMessage + 108, // wParam = element (TASKDIALOG_ELEMENTS), lParam = new element text (LPCWSTR)
            ClickRadioButton = Native.UserMessage + 110, // wParam = Radio Button ID
            EnableButton = Native.UserMessage + 111, // lParam = 0 (disable), lParam != 0 (enable), wParam = Button ID
            EnableRadioButton = Native.UserMessage + 112, // lParam = 0 (disable), lParam != 0 (enable), wParam = Radio Button ID
            ClickVerification = Native.UserMessage + 113, // wParam = 0 (unchecked), 1 (checked), lParam = 1 (set key focus)
            UpdateElementText = Native.UserMessage + 114, // wParam = element (TASKDIALOG_ELEMENTS), lParam = new element text (LPCWSTR)
            SetButtonElevationRequiredState = Native.UserMessage + 115, // wParam = Button ID, lParam = 0 (elevation not required), lParam != 0 (elevation required)
            UpdateIcon = Native.UserMessage + 116  // wParam = icon element (TASKDIALOG_ICON_ELEMENTS), lParam = new icon (hIcon if TDF_USE_HICON_* was set, PCWSTR otherwise)
        }
