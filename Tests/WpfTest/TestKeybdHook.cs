...
namespace WpfTest
{
    ...
    {
        public static void CreateHook(KeyHandler _kh, UInt32 mainThreadId)
        {
            var _this = Process.GetCurrentProcess();
            var mod = _this.MainModule;
            hd = HookFunc;
            kh = _kh;

            hhk = API.SetWindowsHookEx(13, hd, API.GetModuleHandle(mod.ModuleName), 0);

            var threadId = API.GetCurrentThreadId();

            hhk = API.SetWindowsHookEx(14, hd, API.GetModuleHandle(mod.ModuleName),
                mainThreadId);
        ...
        private static IntPtr HookFunc(
            ...
            IntPtr lParam)
        {
            var iwParam = (MessageTypes)wParam.ToInt32();
            if (nCode >= 0 &&
                iwParam == MessageTypes.WM_LBUTTONDOWN ||
                (iwParam == MessageTypes.WM_KEYDOWN ||//0x100 ||
                 iwParam == MessageTypes.WM_SYSKEYDOWN))// 0x104)) //0x100 = WM_KEYDOWN, 0x104 = WM_SYSKEYDOWN
                kh(wParam, lParam);
